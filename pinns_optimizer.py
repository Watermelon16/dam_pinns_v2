...
def compute_physics(n, xi, m, H, gamma_bt, gamma_n, f, C, a1):
    ...
    return sigma, K, A, G, W1, W2, Wt, Fct, Fgt, B, M0, G1, G2, W2_1, W2_2, lG1, lG2, lt, l2, l22, l1, P,sigma_2
...
def optimize_dam_section(H, gamma_bt, gamma_n, f, C, Kc, a1, max_iterations=5000, convergence_threshold=1e-6, patience=50):
    ...
    sigma, K, A, G, W1, W2, Wt, Fct, Fgt, B, M0, G1, G2, W2_1, W2_2, lG1, lG2, lt, l2, l22, l1, P,sigma_2 = compute_physics(n, xi, m, H, gamma_bt, gamma_n, f, C, a1)
...
def create_force_diagram_plotly(H, n, m, xi):
    ...
    def add_arrow(x, y, dx, dy, label,value):
        if label in ['W1']:
            fig.add_annotation(
                x=x, y=y,
                ax=x + dx, ay=y + dy,
    ...
    add_arrow(mid - lG1, H / 3, 0, -5, 'G1',result['G1'])
    add_arrow(mid - lG2, H * (1 - xi) / 3, 0, -5, 'G2', result['G2'])
    add_arrow(mid - lt, 0, 0, -5, 'Wt',result['Wt'])
    add_arrow(mid - l2, H * (1 - xi) + xi * H / 2, 0, -5, "W'2", result['W2_1'])
    add_arrow(mid - l22, 2/3 * H * (1 - xi), 0, -5, 'W"2', result['W2_2'])
    add_arrow(x0 - 2, l1, -5, 0, 'W1', result['W1'])
